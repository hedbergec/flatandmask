...
input_file <- if (length(args) >= 1) args[[1]] else "\\\\Mac/Home/Dropbox/Work/Consult/Consult.Katz/ppd/flatandmask/test_data/test_case_sensitive_values.csv"
output_folder <- if (length(args) >= 2) args[[2]] else "\\\\Mac/Home/Dropbox/Work/Consult/Consult.Katz/ppd/flatandmask/test_output/r_correspondence_1.5.0_regression_key/case-sensitive-values-csv"
secret_key <- if (length(args) >= 3) args[[3]] else "flatandmask-regression-key-v1"
mask_fields <- "root.Value"
