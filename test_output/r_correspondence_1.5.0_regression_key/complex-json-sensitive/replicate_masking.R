...
input_file <- if (length(args) >= 1) args[[1]] else "\\\\Mac/Home/Dropbox/Work/Consult/Consult.Katz/ppd/flatandmask/example data/complex3.json"
output_folder <- if (length(args) >= 2) args[[2]] else "\\\\Mac/Home/Dropbox/Work/Consult/Consult.Katz/ppd/flatandmask/test_output/r_correspondence_1.5.0_regression_key/complex-json-sensitive"
secret_key <- if (length(args) >= 3) args[[3]] else "flatandmask-regression-key-v1"
mask_fields <- c("firstName", "lastName", "email", "phone", "contacts.name", 
"contacts.email", "contacts.phone", "company.headquarters.street", 
"company.headquarters.city", "addresses.street", "addresses.city", 
"paymentMethods.cardNumber", "paymentMethods.cardholderName", 
"paymentMethods.billingAddress", "paymentMethods.accountHolderName"
)
