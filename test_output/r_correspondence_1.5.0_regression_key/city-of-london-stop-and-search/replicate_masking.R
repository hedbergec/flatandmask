...
input_file <- if (length(args) >= 1) args[[1]] else "\\\\Mac/Home/Dropbox/Work/Consult/Consult.Katz/ppd/flatandmask/example data/2026-02-city-of-london-stop-and-search.csv"
output_folder <- if (length(args) >= 2) args[[2]] else "\\\\Mac/Home/Dropbox/Work/Consult/Consult.Katz/ppd/flatandmask/test_output/r_correspondence_1.5.0_regression_key/city-of-london-stop-and-search"
secret_key <- if (length(args) >= 3) args[[3]] else "flatandmask-regression-key-v1"
mask_fields <- c("Gender", "Age range", "Self-defined ethnicity", "Officer-defined ethnicity"
)
