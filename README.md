...
- [Windows Regression Testing](#windows-regression-testing)
- [CSV-Specific Functionality](#csv-specific-functionality)
- [Examples](#examples)
- [Merging Masked Data with IDs](#merging-masked-data-with-ids)
- [Security Considerations](#security-considerations)
- [Troubleshooting](#troubleshooting)
- [Building the Windows Program](#building-the-windows-program)
- [Files Structure](#files-structure)
- [R Implementation (JsonCSVMaskr)](#r-implementation-jsoncsvmaskr)
...
When using the **PowerShell GUI** or **R Shiny GUI**, the tool automatically creates a timestamped subfolder inside your selected output directory:

```
Your_Selected_Output_Folder/
└── Masked_Data_GUI_2026-05-01_15-18-07/  # ← Timestamped subfolder
    ├── data.csv                          # Masked data
    ├── masking_key.csv                   # Original → masked mapping
    ├── sample_masked.json                # Masked JSON (if applicable)
    ├── replicate_masking.ps1             # PowerShell replication script
    ├── DataMaskingTool.ps1               # Matching tool source
    └── ...                               # Additional normalized tables
```

**Format**: `Masked_Data_GUI_YYYY-MM-DD_HH-MM-SS`

### Direct API Calls vs GUI Usage

- **GUI Applications** (PowerShell GUI, R Shiny GUI):
  - Create timestamped subfolders for organization
  - All outputs go into the timestamped folder
  - Replication scripts default to the same timestamped folder

- **Direct API Calls** (PowerShell `Invoke-Masking`, R `datamaskr()`):
  - Use the exact output folder you specify
  - No automatic subfolder creation
  - Outputs go directly to your specified folder

### Replication Scripts and Timestamped Folders

**Important**: Replication scripts generated by GUI runs will **default to using the same timestamped folder** where they were created. This ensures:

- Consistent organization of related masking runs
- Replication outputs stay grouped with original outputs
- Easy identification of related datasets by timestamp

To replicate into a different folder:

```powershell
# Use a different output folder
.\replicate_masking.ps1 -OutputFolder ".\new-masked-output"
```

The replication script remembers the original timestamped folder as its default, but accepts `-OutputFolder` overrides for flexibility.
...
**GUI Runs**: For GUI-generated replication scripts, the default output folder is the **timestamped subfolder** where the script was created (e.g., `Masked_Data_GUI_2026-05-01_15-18-07`). This keeps replication outputs organized with the original GUI run.

Usage:

```powershell
# Replay the original input path saved by the GUI run (outputs to timestamped folder)
.\replicate_masking.ps1

# Replay against a moved or replacement input file (outputs to timestamped folder)
.\replicate_masking.ps1 -InputFile "new_data.json"

# Or with custom parameters (outputs to specified folder):
